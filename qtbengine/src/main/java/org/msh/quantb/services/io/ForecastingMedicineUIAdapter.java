...
public class ForecastingMedicineUIAdapter extends AbstractUIAdapter implements Comparable {
	 ...
	 */
	public Long getBatchesToExpireInt(){
		Long sum = 0L;
	...
	public void setQuantityExpiredLT(Long value) {
		Long oldValue = getQuantityExpiredLT();
	...
	public void setConsumptionCases(Long value) {
		Long oldValue = getConsumptionCases();
	...
	public void setConsumptionNewCases(Integer value) {
		Long oldValue = getConsumptionNewCases();
	...
	public void createResults(ModelFactory modelFactory,
		...
		LocalDate begin = DateUtils.convert(forecastUI.getReferenceDate());
				//new LocalDate(forecastUI.getReferenceDate()); //20160825 seems as appropriate, because "after" below
		for(ForecastingRegimenResultUIAdapter res : resAd){
			...
			LocalDate resDate= DateUtils.convert(res.getFromDate());
					//new LocalDate(res.getFromDate());    //AK 20160822 shift right
	...
	public PricePack getPackOrder(ModelFactory modelFactory){
		...
		if (pack == null){
			if (modelFactory != null){
				pack = modelFactory.cloneOrCreatePricePack(null);
				this.getFcMedicineObj().setPackOrder(pack);
			}else{
				return null;
			}
	...
	public void addOrMergeBatch(ForecastingBatchUIAdapter fBuI) {
		...
		ForecastingBatch fb = ModelFactory.of("").createForecastingBatchExact(fBuI.getExpired());
	...
	public void addOrMergeOrder(ForecastingOrderUIAdapter fOuI) {
		...
		ForecastingOrderUIAdapter newOrder = fOuI.makeClone(ModelFactory.of(""));
	...
	/**
	 * Copy pack prices and pack sizes from oMed
	 * @param oMed
	 */
	public void copyPacks(ForecastingMedicineUIAdapter oMed) {
		PricePack oPack = oMed.getFcMedicineObj().getPackOrder();
		PricePack clone = ModelFactory.of("").cloneOrCreatePricePack(oPack);
		getFcMedicineObj().setPackOrder(clone);
	}
	/**
	 * Create {@link ForecastingMedicineUIAdapter} from kit name 
	 * @param kitName
	 * @return
	 */
	public static ForecastingMedicineUIAdapter ofKit(String kitName) {
		Medicine med =ModelFactory.of("").createMedicine();
		med.setAbbrevName(kitName);
		med.setClassifier(ClassifierTypesEnum.UNKNOWN);
		med.setDosage("");
		med.setForm(MedicineFormEnum.NA);
		med.setFormARV(FormARV.UNKNOWN);
		med.setName(kitName);
		med.setStrength("");
		med.setType(MedicineTypesEnum.UNKNOWN);
		med.setType2018(Classifier2018.UNKNOWN);
		med.setTypeARV(ClassifierARV.UNKNOWN);
		ForecastingMedicine foremed = ModelFactory.of("").createForecastingMedicine(med);
		return new ForecastingMedicineUIAdapter(foremed);
	}
