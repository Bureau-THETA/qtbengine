...
//	}
	 ...
	 * in action since 2020-11-05
	 * @return
	 */
	public List<ForecastingMedicineUIAdapter> getMedicines() {
		List<ForecastingMedicineUIAdapter> ret = new ArrayList<ForecastingMedicineUIAdapter>();
		for (ForecastingMedicine fm : getForecastObj().getMedicines()) {
			ForecastingMedicineUIAdapter fmUI = new ForecastingMedicineUIAdapter(fm);
			ret.add(fmUI);
		}
	 ...
	 * MedicineRegimen fri) { ForecastingMedicine fm = new ForecastingMedicine();
	 * fm.setMedicine(fri.getMedicine()); ForecastingMedicineUIAdapter fmUI = new
	 * ForecastingMedicineUIAdapter(fm); tmp.add(fmUI); }
	 */
	...
	public List<MonthQuantityUIAdapter> getCasesOnTreatment() {
		...
		if (cases != null && !cases.isEmpty() && getForecastObj().isIsOldPercents()) {
			...
			ModelFactory factory = ModelFactory.of("");
	 ...
	 */
	public Long getInitial(MedicineUIAdapter medU) {
		Long res = 0L;
	 ...
	 * @return 0 in nothing
	 */
	public Long getAllTransit(MedicineUIAdapter medicine) {
		Long res = 0L;
	...
	/**
	 * Prepare pre - defined set of ForecastingTotalItem
	 * Only Presenter when create new forecast use it
	 * http://redmine.inka.in.ua/issues/131 2018-08-29
	 * @return
	 */
	public static List<ForecastingTotalItem> preparePredefinedTotalItems(){
		List<ForecastingTotalItem> ret = new ArrayList<ForecastingTotalItem>();
		ret.add(ModelFactory.of("").createForecastingTotalItem(Messages.getString("ForecastingDocumentWindow.tbSummary.items.freight"), new BigDecimal(0)));
		ret.add(ModelFactory.of("").createForecastingTotalItem(Messages.getString("ForecastingDocumentWindow.tbSummary.items.insurance"), new BigDecimal(0)));
		ret.add(ModelFactory.of("").createForecastingTotalItem(Messages.getString("ForecastingDocumentWindow.tbSummary.items.PSI"), new BigDecimal(0)));
		ret.add(ModelFactory.of("").createForecastingTotalItem(Messages.getString("ForecastingDocumentWindow.tbSummary.items.agent"), new BigDecimal(0)));
		ret.add(ModelFactory.of("").createForecastingTotalItem(Messages.getString("ForecastingDocumentWindow.tbSummary.items.custom"), new BigDecimal(0)));
		return ObservableCollections.observableList(ret);
	}
	 ...
	 * @return
	 */
	public long getOldCasesQuantity(ForecastingRegimenUIAdapter fru,
	 ...
	 * @return
	 */
	public long getNewCasesQuantity(ForecastingRegimenUIAdapter fru,
			ModelFactory modelFactory) {
		long res = 0;
	...
	public long getTotalEnrolled() {
		...
		if(!this.isEnrolledCasesPercents()){
			for(ForecastingRegimenUIAdapter frui : this.getRegimes()){
				ret = ret + this.getOldCasesQuantity(frui, ModelFactory.of(""));
			}
		}else{
	...
	public long getTotalExpected() {
		...
		if (!this.isExpectedCasesPercents()){
			for(ForecastingRegimenUIAdapter frui : this.getRegimes()){
				ret = ret + this.getNewCasesQuantity(frui, ModelFactory.of(""));
			}
		}else{
	...
	public static LocalDate calcFirstDate(Calendar referenceDate) {
		LocalDate tmp = DateUtils.convert(referenceDate);
				//new LocalDate(referenceDate); //20180625 really appropriate
		tmp = tmp.plusDays(1);
		return tmp;
	}
	...
	public ForecastUIAdapter copyParameters(){
		...
		fcUi.setQtbVersion(getQtbVersion());
		return fcUi;
	}
	...
	/**
	 * Get max stock as is if not zero, or 30(infinity) if zero
	 * @return
	 */
	...
	public MonthUIAdapter getIniMonth() {
		return new MonthUIAdapter(ModelFactory.of("").createMonth(getIniDate().get(Calendar.YEAR), getIniDate().get(Calendar.MONTH)));
	}
	/**
	...
	public MonthUIAdapter getLeadTimeEndMonth() {
		return new MonthUIAdapter(ModelFactory.of("").createMonth(getLeadTimeEnd().get(Calendar.YEAR),
	 ...
	 * @param forecast
	 */
	public void refreshMedicinesInFc() {
		...
		for (ForecastingRegimenUIAdapter r : getRegimes()) {
		...
		for (ForecastingMedicineUIAdapter fm : getMedicines()) {
		...
		for (MedicineUIAdapter m : medicinesToAdd) {
			ForecastingMedicine fm = ModelFactory.of("").createForecastingMedicine(m.getMedicine());
			ForecastingMedicineUIAdapter fmu = new ForecastingMedicineUIAdapter(fm);
			tmp.add(fmu);
		}
		setMedicines(tmp);
...
/**
 * sign from where the calculation was started.
 * true - collector, false - quanTB (default)
 * @return
 */
	...
	/**
	 * Is this forecasrt has attrition coeff
	 * @return
	 */
	public boolean hasAttrition() {
		boolean all100 = true;
		if(getMedicines()!=null) {
			for(ForecastingMedicineUIAdapter fmui : getMedicines()) {
				BigDecimal hundred = new BigDecimal(100);
				BigDecimal enro = fmui.getAdjustmentEnrolled();
				BigDecimal expe = fmui.getAdjustmentExpected();
				all100 = all100 && enro.equals(hundred) && expe.equals(hundred);
			}
		}
		return !all100;
	}
