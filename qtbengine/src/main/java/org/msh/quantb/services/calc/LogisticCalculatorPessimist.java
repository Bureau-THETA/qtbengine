...
public class LogisticCalculatorPessimist implements LogisticCalculatorI {
	...
	/**
	 * For any given medicine:
	 * <ul>
	 * <li> Clean demands related variables for each forecast month
	 * <li> Split all consumptions to regular and accelerated
	 * <li> 
	 * </ul>
	 * For all monthsClean up demands related variables
	...
	public void exec(List<MedicineConsumption> medConsList) {
		if(medConsList.size()>0){
			...
			for(MedicineConsumption mC : medConsList){
				...
				dCalc.setCollector(getForecastUI().getCollector());
				dCalc.exec();
	...
	/**
	 * Set min and max stock for position given
