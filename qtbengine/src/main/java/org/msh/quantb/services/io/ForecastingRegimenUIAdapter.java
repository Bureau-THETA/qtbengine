...
public class ForecastingRegimenUIAdapter extends AbstractUIAdapter implements Comparable{
	...
	public ForecastingRegimenUIAdapter(ForecastingRegimen fcRegimenObj) {
		...
		if(this.fcRegimenObj.getOriginal().getKit()==null) {
			this.fcRegimenObj.getOriginal().setKit(ModelFactory.of("").createKit());
		}
	}
	 ...
	 * @return
	 */
	public KitDefinitionUIAdapter getKitUI() {
		if(this.fcRegimenObj.getOriginal().getKit()==null) {
			this.fcRegimenObj.getOriginal().setKit(ModelFactory.of("").createKit());
		}
		return new KitDefinitionUIAdapter(this.fcRegimenObj.getOriginal().getKit());
	}


	public void setKitUI(KitDefinitionUIAdapter kitUI) {
		KitDefinitionUIAdapter oldValue = getKitUI();
		this.getFcRegimenObj().getOriginal().setKit(kitUI.getKitDefinition());
		firePropertyChange("kitUI", oldValue, getKitUI());
	}
