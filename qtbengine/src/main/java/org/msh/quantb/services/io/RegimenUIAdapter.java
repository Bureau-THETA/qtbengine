...
public class RegimenUIAdapter extends AbstractUIAdapter implements Comparable<RegimenUIAdapter> {
	...
	public boolean equals(Object _another){
		if (super.equals(_another)){
			...
			return this.getBusinessKey().equals(another.getBusinessKey());
			/*
			return this.getName().equals(another.getName()) &&
					this.getCompositions().equals(another.getCompositions()); //until 2020-10-29
	...
	public PhaseUIAdapter addPhase() {
		...
		Phase phase = ModelFactory.of("").createPhase();
	...
	public void shiftPhases(Integer order) {
		...
		if (phases.size() >=1){
			...
			Phase inten = ModelFactory.of("").createPhase();
			inten.setOrder(1);
		...
		if (phases.size()>=1){
			...
			Phase conti = ModelFactory.of("").createPhase();
			conti.setOrder(2);
	...
	public RegimenUIAdapter makeClone() {
		//create new regimen
		Regimen regObj = ModelFactory.of("").createRegimen(this.getName()+" - " + Messages.getString("Regimen.copy"),
	...
	/**
	 * Currently the business key is  consumption
	 * @return
	 */
