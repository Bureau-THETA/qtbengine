...
public class ForecastMergeControl {
	...
	public ForecastUIAdapter merge() {
		...
		for(ForecastUIAdapter fcUi : getForecasts()){
			...
			copyPackPrices(fcMerged, fcUi); //http://redmine.inka.in.ua/issues/126 20180926
		}
		fcMerged.setMaxStock(1); //http://redmine.inka.in.ua/issues/126 20180926
		fcMerged.setMaxStock(3); //http://redmine.inka.in.ua/issues/126 20180926
		return fcMerged;
	}


	/**
	 * Copy pack sizes and pack prices from the first forecast in a list
	 * Redmine #126, 2018-09-12
	 * @param trgForecast
	 * @param srcForecast
	 */
	private void copyPackPrices(ForecastUIAdapter trgForecast, ForecastUIAdapter srcForecast) {
		for(ForecastingMedicineUIAdapter medUi : trgForecast.getMedicines()){
			ForecastingMedicineUIAdapter firstMedUi = srcForecast.getMedicine(medUi.getMedicine());
			if(firstMedUi != null){
				PricePack srcPack = firstMedUi.getFcMedicineObj().getPackOrder();
				PricePack trgPack = medUi.getFcMedicineObj().getPackOrder();
				if(isPackEmpty(trgPack) && !isPackEmpty(srcPack)){
					medUi.getFcMedicineObj().setPackOrder(srcPack);
				}
			}
		}
	}
	...
	/**
	 * Is this pack contains all data
	 * @param pack
	 * @return
	 */
	private boolean isPackEmpty(PricePack pack) {
		if(pack != null){
			boolean ret =
					pack.getAdjust().equals(BigDecimal.ZERO)
					|| pack.getAdjustAccel().equals(BigDecimal.ZERO)
					|| pack.getPack()==0
					|| pack.getPackAccel()==0
					|| pack.getPackPrice().equals(BigDecimal.ZERO)
					|| pack.getPackPriceAccel().equals(BigDecimal.ZERO);
			return ret;
		}else{
			return true;
		}
	...
	private ForecastUIAdapter prepareEmpty() {
		ForecastUIAdapter fcUi = new ForecastUIAdapter(ModelFactory.of("").createForecasting(""));
	...
	private void addQuantities(ForecastUIAdapter fromFc, ForecastUIAdapter toFc) {
		for(ForecastingRegimen srcReg : fromFc.getForecastObj().getRegimes()){
			...
			// enrolled
			if(trgReg!=null) {																		//changed 2022-01-27 Merge null pointer
	...
	private void prepareQuantAndMeds(ForecastUIAdapter fcMerged) {
		fcMerged.refreshMedicinesInFc(); //create medicines list
