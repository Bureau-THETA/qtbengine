...
public class ForecastingResultUIAdapter extends AbstractUIAdapter implements Comparable<ForecastingResultUIAdapter> {
	...
	/**
	 * copy cases quantity and medicine consumptions from all regimes for a particular day
...
/*				if(fm.getMedicine().getNameForDisplayWithAbbrev().contains("penem") && frr.getFromDay()==17 && frr.getMonth().getYear()==2014 && frr.getMonth().getMonth()==4){
	 ...
	 * @param newCons
	 */
	public void addNewCons(BigDecimal newCons) {
	...
	/**
	 * Returns true if at least one value is not zero
	 * @return
	 */
	public boolean hasData() {
		return
		getConsNew().longValue()>0L
		|| getConsOld().longValue()>0L
		|| getDispensing().longValue()>0L
		|| getExpired().longValue()>0L
		|| getInOrders().longValue()>0L
		|| getMedicineCons().longValue()>0L
		|| getMissing().longValue()>0L
		|| getNewCases().longValue()>0L
		|| getOldCases().longValue()>0L
		|| getOrderExpired().longValue()>0L;
	}
