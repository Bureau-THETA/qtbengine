...
public class DeliveryCalculatorP implements DeliveryCalculatorI {
	...
	public void exec() {
		...
		saveIdealAndClean();
		execIt(false);
		adjustFromOrder(getForecastMedicineUI().getFcMedicineObj().getPackOrder());	//2021.06.04 Play again but without adjustment!
	...
	/**
	 * 2021-06-04. We removed adjusted packs calculation from here!
	 * @param pricePack
	 */
	private void adjustFromOrder(PricePack pricePack) {
		if(pricePack!=null) {
		for(ConsumptionMonth cM : getConsumptions()){
			...
			cM.setPacks(new BigDecimal(pack));
		}
		}
	}
	...
	private BigDecimal calcDelivery(ConsumptionMonth cM) {
		int dateCM=cM.getMonth().getYear()*12+cM.getMonth().getMonth();
		LocalDate current=LocalDate.now();
		int dateCurr=current.getYear()*12+current.getMonthValue();
		if(isCollector()&& dateCM<dateCurr){
