...
public class MedicationUIAdapter extends AbstractUIAdapter implements Comparable<MedicationUIAdapter> {
	...
	public MedicationUIAdapter createClone() {
		MedicineUIAdapter mUi = this.getMedicine().createClone(ModelFactory.of(""));
		MedicineRegimen mR = ModelFactory.of("").createMedication(mUi.getMedicine(), this.getDuration(),
