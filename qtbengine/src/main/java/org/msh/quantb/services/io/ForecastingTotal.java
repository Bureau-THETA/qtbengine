...
public class ForecastingTotal extends AbstractUIAdapter {
	...
	public void recalcItemsFromOrders() {
		if(getOrderTotal() == ALL_TOTAL){
			...
			firePropertyChange("orderGrandTotal", BigDecimal.ZERO, getOrderGrandTotal());
			firePropertyChange("formattedGrandTotal", null, getFormattedGrandTotal());
			firePropertyChange("medTotal", BigDecimal.ZERO, getMedTotal());
			firePropertyChange("formattedTotal", "", getFormattedTotal());
			firePropertyChange("formattedGrandMedTotal", "", getFormattedGrandMedTotal());
			firePropertyChange("formattedGrandTotal", "", getFormattedGrandTotal());
		}
	...
	public void setOrderTotal(int orderTotal) {
		BigDecimal oldGrand = getOrderGrandTotal();
		String oldGrandS = getFormattedGrandTotal();
		BigDecimal oldMedTotal = getMedTotal();
		this.orderTotal = orderTotal;
		firePropertyChange("orderGrandTotal", oldGrand, getOrderGrandTotal());
		firePropertyChange("formattedGrandTotal", oldGrandS, getFormattedGrandTotal());
		firePropertyChange("medTotal", oldMedTotal, getMedTotal());
		firePropertyChange("formattedTotal", "", getFormattedTotal());
		firePropertyChange("formattedGrandMedTotal", "", getFormattedGrandMedTotal());
		firePropertyChange("formattedGrandTotal", "", getFormattedGrandTotal());
	}
	...
	public String getFormattedGrandTotal(){
		...
		//return s+" "+ formatter.format(getCostOrderTotal())+" (USD/$)";
	}
	/**
	...
	/**
	 * copy all pack prices from accel to regular 2018-09-05
	 */
	public void copyAdjCoeffFromAccel() {
		for(ForecastingTotalMedicine med : getMedItems()){
			if(med.getAdjustItAccel().compareTo(BigDecimal.ZERO)>0){
				med.setAdjustIt(med.getAdjustItAccel());
			}
		}
	}

	/**
	 * copy all pack prices from regular to accel 2018-09-05
	 */
	public void copyAdjCoeffFromRegular() {
		for(ForecastingTotalMedicine med : getMedItems()){
			if(med.getAdjustIt().compareTo(BigDecimal.ZERO)>0){
				med.setAdjustItAccel(med.getAdjustIt());
			}
		}
	}
	...
	/**
	 * All medicines have pack prices and pack sizes. Medicines with adjustment coefficient zero% this rule will ignore
	 * @return
	 */
	public boolean hasAllCosts(){
		boolean ret = true;
		for(ForecastingTotalMedicine fmed : getMedItems()){
			Long quant = fmed.getAccelQuant();
			BigDecimal price = fmed.getPackPriceAccel();
			BigDecimal adj=fmed.getAdjustItAccel();
			Integer packSize = fmed.getPackSizeAccel();
			if(getOrderTotal() == REGULAR_TOTAL){
				quant=fmed.getRegularQuant();
				price = fmed.getPackPrice();
				adj=fmed.getAdjustIt();
				packSize= fmed.getPackSize();
			}
			if(!adj.equals(BigDecimal.ZERO) ) {
				if(quant>0 && (price.equals(BigDecimal.ZERO) || packSize==0)){
					if(getOrderTotal() == REGULAR_TOTAL){
						fmed.setRegChecked(false);
					}else {
						fmed.setAccelChecked(false);
					}
					ret=false;
				}else {
					if(getOrderTotal() == REGULAR_TOTAL){
						fmed.setRegChecked(true);
					}else {
						fmed.setAccelChecked(true);
					}
				}
			}else {
				if(getOrderTotal() == REGULAR_TOTAL){
