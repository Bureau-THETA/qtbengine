...
public class ForecastingOrderUIAdapter extends AbstractUIAdapter implements Comparable<ForecastingOrderUIAdapter>  {
	...
	public int compareTo(ForecastingOrderUIAdapter o) {
		...
		if (res == 0) res = this.hashCode() - o.hashCode();
		return res;
	}
	/**
	...
	public void setBatchInclude(Boolean value){
		if(getBatch()!=null){
			...
			Long oldValue1 = getBatchQuantity();
	...
	public void setBatchQuantity(Long value){
		Long oldValue = getBatch().getQuantity();
		getBatch().setQuantity(value);
		firePropertyChange("batchQuantity", oldValue, getBatchQuantity());
	}

	public void setGdf(Boolean value) {
		Boolean oldValue = getForecastingOrderObj().getBatch().isGdf();
		getForecastingOrderObj().getBatch().setGdf(value);
		firePropertyChange("gdf",oldValue,getGdf());
	}
	
	public Boolean getGdf() {
		return getForecastingOrderObj().getBatch().isGdf();
	}
