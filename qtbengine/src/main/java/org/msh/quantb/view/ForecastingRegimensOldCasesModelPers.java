...
public class ForecastingRegimensOldCasesModelPers extends AbstractTableModel implements HasRegimenData {

	private static final long serialVersionUID = -6681431458487211767L;
	/**
	 * Constructor
	 * 
	 ...
	 * @param forecastingDocumentPanel 
	 */
	public ForecastingRegimensOldCasesModelPers(ForecastUIAdapter fcUI, HasCalculationDetails forecastingDocumentPanel) {
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		if (columnIndex == 1) {
			return String.format("%4d",rowIndex+1)+". " +data.get(rowIndex).getRegimen().getNameWithForDisplay();
		} else {			
			return data.get(rowIndex).getPercentCasesOnTreatment();
		}
	}
	...
	public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
		...
		if(columnIndex == 2){
			data.get(rowIndex).setPercentCasesOnTreatment((Float) aValue);
			this.mainTabPane.setVisibleCalculationDetailsTabs(false);
		}
	}
	...
	public String getColumnName(int column) {
		...
		if(column==0){
			return "<html>"+
					Messages.getString("ForecastingDocumentWindow.tbParameters.SubTab.NewCases.disable")+
					"<br>&nbsp</html>";
		}
		return "";
	}
